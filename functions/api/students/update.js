...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const ENCRYPTED_FIELDS = ['rut', 'diagnosis', 'guardian', 'observations'];

    const stmts = [];

    for (const [inputKey, dbColumn] of Object.entries(FIELD_MAP)) {
      if (data[inputKey] !== undefined) {
        let value = data[inputKey];
        // Cifrar campos sensibles
        if (ENCRYPTED_FIELDS.includes(inputKey) && env.ENCRYPTION_KEY) {
